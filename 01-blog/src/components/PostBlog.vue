<template>
  <article class="post">
    <div>
      ...
      <svg width="25" height="25" viewBox="0 0 200 200" fill="none" class="ic-click" :class="{ active: isFavorite }"
        @click="isFavorite = !isFavorite" xmlns="http://www.w3.org/2000/svg">
        <path
...
export default {
  ...
  methods: {
    ...
    nameShortMonth(mes) {
      const nomesMes = ['jan', 'fev', 'mar', 'abr', 'maio', 'jun', 'jul', 'ago', 'set', 'out', 'nov', 'dez'];
      return nomesMes[mes];
    }
  },
  computed: {
    dateFormat() {
      const date = new Date(this.post.dataPostagem);
      date.toLocaleDateString('pt-br');

      const day = this.addZeroToDate(date.getDate());
      const month = this.nameShortMonth(date.getMonth() + 1);
...
.post {
  ...
  div {
    ...
    span {
      color: $dark-20;
      font: $weight-400 $size-small $family-primary;
      line-height: 1.25rem;
    }
  ...
  h2 {
    color: $dark-10;
    font: $weight-400 $size-title $family-primary;
    margin-bottom: .6rem;
    line-height: 2rem;
  }
